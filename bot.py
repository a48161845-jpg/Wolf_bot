...
LARETS_PRIZES = [
    {"label": "2 🧪 зелья силы", "weight": 25, "grant": {"potion": ("strength", 2)}},
    ...
    {"label": "30 🀄 рун + 1 💍 кольцо", "weight": 2, "grant": {"runes": 30, "rings": 1}},
...
QUEST_DEFS = {
    # по запросу: доллары за квесты больше не даются
    "feed_pet": {"target": 1, "title": "Покормить волка", "reward": dict(xp=5)},
    "send_forest": {"target": 1, "title": "Отправить питомца в лес", "reward": dict(xp=5)},
    "send_hunt": {"target": 1, "title": "Отправить питомца на охоту", "reward": dict(xp=5)},
    "use_bonus": {"target": 1, "title": "Забрать ежедневный бонус", "reward": dict(keys=5)},
    "cook_meat": {"target": 2, "title": "Приготовить 2 порции мяса на костре", "reward": dict(keys=3)},
    "hunt_3_times": {"target": 3, "title": "Отправиться на охоту 3 раза", "reward": dict(keys=10)},
    "collect_bones_5": {"target": 5, "title": "Найти 5 косточек", "reward": dict(xp=3)},
    "win_fight": {"target": 1, "title": "Победить в бою", "reward": dict(runes=5)},
    "pet_level_up": {"target": 1, "title": "Повысить уровень питомца", "reward": dict(xp=5)},
}
...
def profile_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎁 Ларец", callback_data="larets_screen")],
...
# =========================================================
#  РАНГИ ИГРОКА — зависят от уровня волка (по запросу)
# =========================================================
# Список (мин. уровень, название ранга) по возрастанию — берётся последний
# порог, который игрок уже прошёл.
...
def get_rank_title(level: int | None) -> str:
    """Ранг по уровню живого волка. Если волка нет/погиб — базовый ранг."""
...
def profile_text(user: sqlite3.Row, pet: sqlite3.Row | None) -> str:
    if pet and pet["alive"]:
        ...
        rank_title = get_rank_title(pet["level"])
...
async def claim_periodic_bonus(user_id: int) -> str:
    ...
    # по запросу: доллары за бонус больше не даются
    update_user(
        user_id,
        xp=user["xp"] + xp_gain,
...
async def cmd_vydat(message: Message):
    ...
    u = message.from_user
    if not is_admin(u.id):
        await reply_or_send(message, "❌ Эта команда доступна только администраторам бота.")
        return
...
async def cmd_grant_admin(message: Message):
    u = message.from_user
    # БАГФИКС: раньше выдавать права мог только тот, чей ID прописан в
    # переменной окружения ADMIN_IDS. Если её вообще не задали (обычная
    # ситуация при первом запуске бота), ADMIN_IDS оказывался пустым
    # множеством — и АБСОЛЮТНО НИКТО не мог никогда получить права админа
    # через бота, команда просто молча ничего не делала (return без ответа).
    # Теперь: если в системе ещё нет ни одного админа (ни из ADMIN_IDS,
    # ни выданных ранее через !admin) — команду может один раз выполнить
    # любой пользователь, чтобы "поднять" самого первого админа. Как только
    # админ появился, это окно бутстрапа закрывается само собой.
    bootstrap = len(get_all_admin_ids()) == 0
    if not bootstrap and u.id not in ADMIN_IDS:
        await reply_or_send(
            message,
            "❌ Выдавать права администратора может только владелец бота "
            "(ID, прописанный в переменной окружения ADMIN_IDS).",
        )
        return
    ...
    if target_id is None:
        if bootstrap:
            # без аргументов при бутстрапе — назначаем админом самого себя
            target_id = u.id
        else:
...
async def cb_maze_move(callback: CallbackQuery):
    ...
    if progress >= MAZE_STEPS_TO_WIN:
        # по запросу: доллары за лабиринт больше не даются
        maze_reward = dict(runes=MAZE_WIN_RUNES)
...
def larets_status_kb(can_open: bool) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="⬅ Назад в профиль", callback_data="back_profile")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def larets_text(user: sqlite3.Row) -> str:
    text = (
        "🎁 ЛАРЕЦ\n————————————\n"
...
async def _send_larets_screen(message: Message, user_id: int):
    ...
    sent = await reply_or_send(message, larets_text(user), reply_markup=larets_status_kb(can_open))
    remember_owner(sent.chat.id, sent.message_id, user_id)
...
@dp.callback_query(F.data == "larets_screen")
async def cb_larets_screen(callback: CallbackQuery):
    if not await _owner_allowed(callback):
        return
    u = callback.from_user
    user = get_or_create_user(u.id, u.username or "", u.full_name)
    can_open = user["larets"] >= 1 and user["keys"] >= LARETS_OPEN_KEY_COST
    await safe_edit_or_send(callback.message, larets_text(user), larets_status_kb(can_open), owner_id=u.id)
    await callback.answer()
